...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($genreModel->ajouter($nom_genre, $description)) {
            header('Location: index.php?message=Genre ajouté avec succès&type=success');
            exit;
        } else {
            throw new Exception("Erreur lors de l'ajout du genre");
        }
    } catch (Exception $e) {
...
<head>
    ...
    <title>Ajouter un Genre - Bibliothèque</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <header>
        <div class="container">
            <h1 style="text-align: center; margin-bottom: 20px;">➕ Ajouter un Genre</h1>
            <nav>
                <ul>
            ...
            <form method="POST">
                <div class="form-group">
                    ...
                    <input type="text" id="nom_genre" name="nom_genre" required 
                           ...
                           value="<?= isset($_POST['nom_genre']) ? htmlspecialchars($_POST['nom_genre']) : '' ?>">
                </div>

                <div class="form-group">
                    <label for="description">Description</label>
                    <textarea id="description" name="description" 
                              placeholder="Description optionnelle du genre..."><?= isset($_POST['description']) ? htmlspecialchars($_POST['description']) : '' ?></textarea>
                    <div class="help-text">
